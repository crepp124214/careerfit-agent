...
const resumeId = ref(route.params.id as string)
...
<template>
  <section class="resume-detail-view" role="main" aria-label="简历详情">
    ...
    <div v-else-if="resume" class="resume-detail-view__card">
      <h1 class="resume-detail-view__title">{{ resume.name }}</h1>
      <div v-if="resume.content" class="resume-detail-view__content">
        <h2 class="resume-detail-view__section-title">简历内容</h2>
        <pre class="resume-detail-view__content-text">{{ resume.content }}</pre>
      </div>
      <p class="resume-detail-view__meta">创建于 {{ resume.createdAt }}</p>
    </div>
